...
const shoppingCart = createReducer(initialState, (builder) => {
	builder.addCase(addItemInCart, (state, action) => {
		...
		const deleteDublicate = Array.from(new Set(pushToCart.map((a) => a?.id))).map((id) => {
	...
	builder.addCase(deleteCountItem, (state, action) => {
		...
		const updateCountCardItem = state.cartData[dishes].map((i) =>
			i.id === id ? { ...i, count: count, totalSumm: i.totalSumm - i.price } : i
		);
		const updateCountShoppingCart = state.shoppingCart.map((i) =>
			i.id === id ? { ...i, count: count, totalSumm: i.totalSumm - i.price } : i
		);
